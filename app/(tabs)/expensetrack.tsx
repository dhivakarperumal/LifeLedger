...
export default function ExpenseTrack() {
  ...
  const EXPENSE_FILTER_GROUPS = [
    {
      key: "category",
      label: "Category",
      options: ["Food", "Travel", "Shopping", "Bills", "Health", "Fun", "UPI"],
      multi: true,
    },
    {
      key: "payment",
      label: "Payment Method",
      options: ["Cash", "UPI", "Credit Card", "Debit Card", "Net Banking"],
      multi: true,
    },
  ];
  const [filterVisible, setFilterVisible] = useState(false);
  const [filterState, setFilterState] = useState<FilterState>(
    defaultFilterState(EXPENSE_FILTER_GROUPS),
  );
  ...
  useEffect(() => {
    ...
    if (payFilter.length > 0) {
      result = result.filter((item) =>
        payFilter.some((p) =>
          item.paymentMethod?.toLowerCase().includes(p.toLowerCase()),
        ),
      );
    }
  ...
  const pickDocument = async () => {
    try {
      Alert.alert("Upload Receipt", "Choose a file type", [
        {
  ...
  const addExpense = async () => {
    ...
    try {
      ...
      const remaining =
        selectedTransfer.remainingAmount ?? selectedTransfer.amount ?? 0;
  ...
  const handleBarCodeScanned = ({ data }: { data: string }) => {
    ...
    try {
      if (data.startsWith("upi://pay")) {
        ...
        Alert.alert(
          "Invalid QR",
          "This QR code is not a valid UPI payment code.",
  ...
  const confirmAndPay = async () => {
    ...
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          "Payment Initiated",
          "Check your UPI app to complete payment. Don't forget to 'Save Expense' here!",
        );
      } else {
  ...
  const getCategoryStyles = (cat: string) => {
    ...
    if (
      ...
      categoryName.includes("cab") ||
      categoryName.includes("petrol") ||
      categoryName.includes("fuel")
    )
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    overflow: "hidden",
                    borderWidth: 2,
                    borderStyle: receipt ? "solid" : "dashed",
                    borderColor: receipt ? "#e5e7eb" : "#d1d5db",
                ...
                <View style={{ flexDirection: "row", marginBottom: 24 }}>
                  <View style={{ flex: 1, marginRight: 12 }}>
                    ...
                    <DarkDropdown
                      value={selectedTransfer?.id ?? null}
                      placeholder="Select"
                      onChange={(value) =>
                        setSelectedTransfer(
                          transferList.find((i: any) => i.id === value) || null,
                        )
                      }
                      options={[
                        { label: "Select", value: null },
                        ...transferList.map((t: any) => ({
                          label: `${t.name} - ₹${t.remainingAmount ?? t.amount}`,
                          value: t.id,
                        })),
                      ]}
                    />
                  ...
                  {PRESET_CATEGORIES.map((cat) => (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 16,
                        backgroundColor:
                          category === cat.name ? "#2f5d34" : "#f8fafc",
                        flexDirection: "row",
                        alignItems: "center",
                        borderWidth: 1.5,
                        borderColor:
                          category === cat.name ? "#2f5d34" : "#f0f0f0",
                      }}
                    >
                      <View
                        style={{
                      ...
                      <Text
                        style={{
                          fontWeight: "800",
                          color: category === cat.name ? "white" : "#111827",
                          fontSize: 13,
                        }}
                      >
                        {cat.name}
                ...
                >
                  {loading ? (
                    ...
                    >
                      {editingId ? "Update Detail" : "Confirm Expense"}
                    </Text>
                  )}
                </TouchableOpacity>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <Text
                  style={{ fontSize: 24, fontWeight: "900", color: "#1f2937" }}
                >
                  Payment Details
                </Text>
                <TouchableOpacity
                  onPress={() => setShowUPIDetails(false)}
                  style={{
      ...
      >
        ...
        >
          ...
          >
            <View
              style={{
                backgroundColor: getCategoryStyles(viewingItem?.category).color,
            ...
            >
              ...
              >
                <Ionicons
                  name={getCategoryStyles(viewingItem?.category).icon as any}
                  size={48}
                  color="white"
                />
              </View>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {viewingItem?.paymentMethod || "Cash"}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    ...
                    >
                      {viewingItem?.expenseDate
                        ? formatDateShort(viewingItem.expenseDate)
                        : "Today"}
                    </Text>
                  </View>
                  <View
                  ...
                  >
                    ...
                    >
                      {viewingItem?.expenseDate
                        ? formatTimeShort(viewingItem.expenseDate.toDate())
                        : "--:--"}
                    </Text>
                  </View>
                </View>
        ...
        >
          ...
          >
            ...
            <View style={{ flexDirection: "row", width: "100%" }}>
              ...
              >
                <Text
                  style={{ color: "#4b5563", fontWeight: "800", fontSize: 18 }}
                >
                  Cancel
                </Text>
              </TouchableOpacity>

...
export default function Reports() {
  ...
  const largestExpense = useMemo(
    () => Math.max(0, ...filtered.map((e) => e.amount || 0)),
    [filtered],
  );
  ...
  const trendData = useMemo(() => {
    if (period === "week") {
      return Array.from({ length: 7 }, (_, i) => {
        ...
        const amount = filtered
          .filter((e) => {
            const ed = getDateForItem(e);
            return ed && ed >= ds && ed <= de;
          })
    ...
    if (period === "month") {
      ...
      filtered.forEach((e) => {
        const d = getDateForItem(e);
        if (d)
          weeks[Math.min(3, Math.floor((d.getDate() - 1) / 7))].amount +=
            e.amount || 0;
      });
      return weeks;
    }
    const months = Array.from({ length: 12 }, (_, i) => ({
      label: new Date(2024, i, 1).toLocaleDateString("en-IN", {
        month: "short",
      }),
      amount: 0,
    }));
    filtered.forEach((e) => {
      const d = getDateForItem(e);
      if (d) months[d.getMonth()].amount += e.amount || 0;
    });
    return months;
  }, [filtered, period]);
                ...
                <html>
                    <head>
                        <style>
                            ...
                            th { background-color: #f0fdf4; color: #2f5d34; }
                            .total { font-weight: bold; font-size: 1.4em; text-align: right; margin-top: 20px; color: #2f5d34; }
                        </style>
                    </head>
                    <body>
                        <h1>Life Ledger Expense Report</h1>
                        <div class="info">
                            <strong>Period:</strong> ${period.toUpperCase()}<br/>
                            <strong>Total Expenses:</strong> ${fmt(totalSpent)}<br/>
                        ...
                        <table>
                            ...
                            ${filtered
                              .map(
                                (e) => `
                                <tr>
                                    <td>${getDateForItem(e)?.toLocaleDateString("en-IN") || ""}</td>
                                    <td>${e.name || "Expense"}</td>
                                    <td>${e.category}</td>
                        ...
                        <div class="total">Total: ₹${totalSpent.toLocaleString("en-IN")}</div>
                    </body>
                </html>
            `;
  ...
  const exportToExcel = async () => {
    try {
      ...
      const rows = filtered
        .map((e) => {
          const date = getDateForItem(e)?.toLocaleDateString("en-IN") || "";
          const title = `"${(e.name || "Expense").replace(/"/g, '""')}"`;
          const category = `"${e.category}"`;
          const pm = `"${e.paymentMethod}"`;
          const amount = e.amount || 0;
          return `${date},${title},${category},${pm},${amount}`;
        })
        .join("\n");
  ...
  return (
    ...
    >
      ...
      <View className="bg-gray-900 px-4 py-4 flex-row items-center">
        ...
        <Text className="text-white text-xl font-bold">Expense Reports</Text>
      </View>
      {/* ── Period Tabs ── */}
      <View
        style={{
        ...
        {PERIOD_TABS.map((t) => {
          ...
          return (
            <TouchableOpacity
              key={t.key}
              onPress={() => {
                if (t.key === "custom") setShowCustomModal(true);
                else setPeriod(t.key);
              }}
              style={{
                flex: 1,
                flexDirection: "row",
      ...
      {loading ? (
        ...
        >
          ...
          <Text style={{ color: TEXT_SEC, marginTop: 12, fontWeight: "600" }}>
            Loading report...
          </Text>
        </View>
      ) : (
        ...
        >
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={{ color: TEXT_PRIMARY, fontSize: 24, fontWeight: "900" }}
              >
                {filtered.length}
              </Text>
              <Text
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              <Text
                style={{ color: TEXT_HINT, fontSize: 10, fontWeight: "600" }}
              >
                avg each
              </Text>
            </View>
                  ...
                  <View key={i} style={{ flex: 1, alignItems: "center" }}>
                    ...
                    <View
                      style={{
                        ...
                        height: barH,
                        borderRadius: 6,
                        backgroundColor: isMax ? ACCENT : "#d1fae5",
                      }}
                    />
                    <Text
                      style={{
          ...
          {categoryBreakdown.length > 0 && (
            ...
            >
              ...
              {categoryBreakdown.map(([cat, amt], i) => {
                ...
                const color =
                  CATEGORY_COLORS[cat] || PAY_COLORS[i % PAY_COLORS.length];
                ...
                return (
                  <View
                    ...
                    style={{
                      flexDirection: "row",
                      alignItems: "center",
                      marginBottom: i < topExpenses.length - 1 ? 14 : 0,
                    }}
                  >
                    <View
                      style={{
          ...
          {filtered.length === 0 && (
            <View style={{ alignItems: "center", paddingVertical: 60 }}>
              ...
              <Text
                style={{ color: TEXT_PRIMARY, fontWeight: "800", fontSize: 16 }}
              >
                No data for this period
              </Text>
              <Text style={{ color: TEXT_SEC, fontSize: 13, marginTop: 4 }}>
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text
                style={{
                  color: customFrom ? TEXT_PRIMARY : TEXT_HINT,
                  fontWeight: "700",
                  flex: 1,
                }}
              >
            ...
            >
              ...
              <Text
                style={{
                  color: customTo ? TEXT_PRIMARY : TEXT_HINT,
                  fontWeight: "700",
                  flex: 1,
                }}
              >
            ...
            <TouchableOpacity
              ...
              style={{
                backgroundColor: customFrom && customTo ? ACCENT : "#f1f5f9",
                borderRadius: 20,
                paddingVertical: 18,
                alignItems: "center",
                shadowColor: ACCENT,
                shadowOffset: { width: 0, height: 4 },
                shadowOpacity: customFrom && customTo ? 0.2 : 0,
                shadowRadius: 8,
                elevation: customFrom && customTo ? 4 : 0,
              }}
            >
              <Text
                style={{
                  color: customFrom && customTo ? "white" : TEXT_HINT,
                  fontWeight: "900",
                  fontSize: 15,
                  textTransform: "uppercase",
                  letterSpacing: 2,
